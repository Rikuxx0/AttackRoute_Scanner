...
# --- Configuration ---
# .envファイルは使用せず、Streamlitのsecretsと環境変数を直接参照します。

def get_gemini_api_key():
    """
    Gets the Gemini API key from Streamlit secrets or environment variable.
    """
    try:
        # 1. Try to get from Streamlit secrets first (recommended for Streamlit apps)
        ...
        if api_key:
            ...
            st.error("Gemini APIキーが見つかりません。Streamlit secretsまたは環境変数に設定してください。")
            st.info("設定方法: secrets.tomlファイルに `GEMINI_API_KEY = \"your_key\"` を追加するか、環境変数として設定してください。")
            return None
...
*   専門用語を避け、誰にでも危険性が一目で理解できる平易な言葉で説明してください。
*   最終的に、このシステム全体が直面している最も大きな脅威は何か、そして最悪の場合どのような事態が想定されるかを結論として述べてください。
*   出力はMarkdown形式で、重要な箇所は太字(**)で強調してください。
*   説明は長く説明せずに、簡潔に説明して攻撃シナリオを提示してください。
